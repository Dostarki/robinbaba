...
export function PublicHeader({ legal = false }) {
  ...
  useEffect(() => {
    if (legal) return;
    const ids = ["overview", "token", "bonding", "curve", "mechanics", "rewards"];
  ...
  const links = legal ? [{ href: "/landing", label: "Overview" }, { href: "/intelligence", label: "Intelligence" }] : [{ href: "#overview", label: "Overview", id: "overview" }, { href: "#bonding", label: "Bonding", id: "bonding" }, { href: "#curve", label: "Curve", id: "curve" }, { href: "#rewards", label: "Rewards", id: "rewards" }];
...
const SALE_ABI = ["function indexUsd() view returns (uint256)", "function totalSold() view returns (uint256)", "function currentPriceUsd() view returns (uint256)", "function buyerCount() view returns (uint256)", "function saleOpen() view returns (bool)", "function bondingComplete() view returns (bool)", "function quote(uint256) view returns (uint256,uint256,uint256,uint256)", "function buy() payable"];

function usePublishedSale(amount) {
  const [sale, setSale] = useState({ loading: true, deployment: null, state: null, quote: null, error: "" });
  const load = async () => {
    try {
      const response = await fetch("/api/public/state", { cache: "no-store" });
      const data = await response.json();
      const deployment = data?.sale?.deployment;
      if (!deployment) return setSale({ loading: false, deployment: null, state: null, quote: null, error: "" });
      const ethers = await import("ethers"), provider = new ethers.JsonRpcProvider(deployment.rpcUrl, deployment.chainId), contract = new ethers.Contract(deployment.curveAddress, SALE_ABI, provider);
      const [indexUsd, totalSold, currentPriceUsd, buyerCount, saleOpen, bondingComplete] = await Promise.all([contract.indexUsd(), contract.totalSold(), contract.currentPriceUsd(), contract.buyerCount(), contract.saleOpen(), contract.bondingComplete()]);
      let quote = null;
      try { const [usdAccepted, weiAccepted, tokenOut, refund] = await contract.quote(ethers.parseEther(amount || "0")); quote = { usdAccepted: Number(ethers.formatUnits(usdAccepted, 18)), weiAccepted: ethers.formatEther(weiAccepted), tokenOut: Number(ethers.formatUnits(tokenOut, 18)), refund: ethers.formatEther(refund) }; } catch {}
      setSale({ loading: false, deployment, state: { indexUsd: Number(ethers.formatUnits(indexUsd, 18)), totalSold: Number(ethers.formatUnits(totalSold, 18)), currentPriceUsd: Number(ethers.formatUnits(currentPriceUsd, 18)), buyerCount: Number(buyerCount), saleOpen, bondingComplete }, quote, error: "" });
    } catch { setSale(previous => ({ ...previous, loading: false, error: "The published curve could not be read right now." })); }
  };
  useEffect(() => { load(); const timer = window.setInterval(load, 20_000); return () => window.clearInterval(timer); }, [amount]);
  return { ...sale, reload: load };
}

function BondingDesk() {
  const [amount, setAmount] = useState("0.05");
  const [wallet, setWallet] = useState("");
  const [walletMessage, setWalletMessage] = useState("");
  const quote = useEthQuote();
  const published = usePublishedSale(amount);
  const eth = Math.max(0, Number(amount || 0));
  const usdIn = eth * (quote.rate || 0);
  const math = useMemo(() => quoteCurve(0, usdIn), [usdIn]);
  const live = published.deployment && published.state;
  const liveQuote = published.quote;
  const indexUsd = live ? published.state.indexUsd : 0;
  const progress = Math.min(100, indexUsd / CURVE_TARGET * 100);
  const connect = async () => {
    if (!window.ethereum?.request) { setWalletMessage("Install an EVM wallet to connect here."); return; }
    try {
      const accounts = await window.ethereum.request({ method: "eth_requestAccounts" });
      const address = accounts?.[0] || "";
      setWallet(address);
      if (address && published.deployment) { const desired = `0x${Number(published.deployment.chainId).toString(16)}`; const current = await window.ethereum.request({ method: "eth_chainId" }); if (current !== desired) await window.ethereum.request({ method: "wallet_switchEthereumChain", params: [{ chainId: desired }] }); }
      setWalletMessage(address ? (published.deployment ? "Wallet connected to the published purchase network." : "Wallet connected. A purchase target has not been published yet.") : "No account was returned by the wallet.");
    } catch (error) {
      setWalletMessage(error?.code === 4001 ? "Wallet connection was declined." : "Wallet connection could not be completed.");
    }
  };
  const purchase = async () => {
    if (!published.deployment || !published.state?.saleOpen || !window.ethereum) return;
    try {
      setWalletMessage("Preparing the on-chain quote…");
      const ethers = await import("ethers"), provider = new ethers.BrowserProvider(window.ethereum), signer = await provider.getSigner(), contract = new ethers.Contract(published.deployment.curveAddress, SALE_ABI, signer);
      const value = ethers.parseEther(amount || "0"), onchainQuote = await contract.quote(value);
      if (onchainQuote[1] === 0n || onchainQuote[2] === 0n) throw Error("This amount cannot be accepted by the active curve.");
      const transaction = await contract.buy({ value });
      setWalletMessage(`Transaction submitted: ${transaction.hash.slice(0, 10)}… Waiting for confirmation.`);
      await transaction.wait(); await published.reload(); setWalletMessage("Commitment confirmed on-chain.");
    } catch (error) { setWalletMessage(error?.code === 4001 ? "Transaction was declined in the wallet." : (error?.shortMessage || error?.message || "The commitment could not be completed.")); }
  };
  return <section className="ri-bonding ri-section" id="bonding">
    <div className="ri-bonding-heading"><div><p className="ri-kicker">{live ? `${published.deployment.tokenSymbol} bonding` : "Bonding preview"}</p><ScrambleText as="h2">A transparent entry, not a black box.</ScrambleText></div><ScrambleText as="p">{live ? `This purchase panel is connected to ${published.deployment.label}, selected in the protected admin workspace.` : "Review a curve quote and the sale mechanics in one place. Commitments stay disabled until an administrator publishes a verified contract and network."}</ScrambleText></div>
    <div className="ri-bonding-shell">
      <article className="ri-bonding-index"><div className="ri-bonding-card-top"><span>Curve index</span><b>{live ? (published.state.saleOpen ? "Live" : "Closed") : "Preview"}</b></div><strong>{usd(indexUsd)}</strong><p>of {usd(CURVE_TARGET)} target</p><div className="ri-bonding-rail" aria-label="Bonding progress"><i style={{ width: `${progress}%` }} /></div><div className="ri-bonding-ticks"><span>$0</span><span>$25K</span><span>$50K</span><span>$75K</span><span>$100K</span></div><dl className="ri-bonding-stats"><div><dt>Spot price</dt><dd>{usd(live ? published.state.currentPriceUsd : PRICE_START)}</dd></div><div><dt>Curve supply</dt><dd>200M</dd></div><div><dt>Available now</dt><dd>{compact(live ? Math.max(0, CURVE_SUPPLY - published.state.totalSold) : CURVE_SUPPLY)}</dd></div><div><dt>Wallets in</dt><dd>{live ? compact(published.state.buyerCount) : "—"}</dd></div></dl></article>
      <form className="ri-bonding-form" onSubmit={event => event.preventDefault()}><div className="ri-bonding-card-top"><span>Estimate a commitment</span><b className="ri-status">{live ? (published.state.saleOpen ? "Live" : "Not open") : "Not live"}</b></div><h3>Choose your position on the curve.</h3><p>{live ? `${published.deployment.tokenSymbol} quotes are read from the selected curve contract before a wallet transaction is requested.` : "Estimates use the published curve parameters and the current ETH/USD reference rate."}</p><label className="ri-bonding-label" htmlFor="bonding-amount">Amount in ETH</label><div className="ri-bonding-amount"><input id="bonding-amount" type="number" inputMode="decimal" min="0" step="0.001" value={amount} onChange={event => setAmount(event.target.value)} /><span>ETH</span></div><div className="ri-bonding-chips" aria-label="Quick amount selection">{["0.01", "0.05", "0.25", "1"].map(value => <button key={value} type="button" className={amount === value ? "is-selected" : ""} onClick={() => setAmount(value)}>{value}</button>)}</div><div className="ri-bonding-quote"><div><span>Estimated allocation</span><b>{compact(liveQuote ? liveQuote.tokenOut : math.tokens)} {published.deployment?.tokenSymbol || "ROB"}</b></div><div><span>Average entry</span><b>{liveQuote?.tokenOut ? usd(liveQuote.usdAccepted / liveQuote.tokenOut) : math.tokens ? usd(math.averagePrice) : "—"}</b></div><div><span>Next curve price</span><b>{usd(live ? published.state.currentPriceUsd : math.nextPrice)}</b></div></div><p className={`ri-bonding-rate ${quote.error || published.error ? "is-error" : ""}`}>{published.error || (liveQuote?.refund !== "0.0" && Number(liveQuote?.refund || 0) > 0 ? `Final-buy refund estimate: ${liveQuote.refund} ETH · ` : "")}{quoteStatus(quote)}</p>{!wallet ? <button className="ri-bonding-connect" type="button" onClick={connect}>Connect wallet</button> : <button className="ri-bonding-connect" type="button" disabled={!live || !published.state.saleOpen} onClick={purchase}>{live && published.state.saleOpen ? `Buy ${published.deployment.tokenSymbol}` : "Commitments are not open"}</button>}{walletMessage && <p className="ri-bonding-message" aria-live="polite">{walletMessage}</p>}<p className="ri-bonding-disclosure">{live ? `Network: ${published.deployment.chainName}. Recipient: ${shortAddress(published.deployment.curveAddress)}. Check the amount and contract before signing.` : "No commitment can be sent until an administrator selects and publishes a saved deployment."}</p></form>
    </div>
    <div className="ri-bonding-tape"><div><span>01</span><h3>Commit feed</h3></div><p>Waiting for a verified deployment before reading on-chain commitments.</p><small>On launch, this area will show truncated wallet identifiers, committed amount and confirmed block time—not fabricated activity.</small></div>
  </section>;
}
...
export function LandingV2() { return <><LandingBackdrop/><PublicHeader/><main className="ri-landing"><BondingDesk/><section className="ri-hero ri-section" id="overview"><div className="ri-hero-copy"><p className="ri-kicker">Token research, with context</p><ScrambleText as="h1">Look beyond the ticker.</ScrambleText><ScrambleText as="p">Review token security signals, creator activity and the evidence behind each score—before deciding what to do next.</ScrambleText><div className="ri-hero-actions"><a href="/intelligence" className="ri-button ri-button-primary">Open Intelligence <span>↗</span></a><a href="#token" className="ri-button ri-button-quiet">Explore the token <span>↓</span></a></div></div><EvidenceField/></section><TokenOverview/><CurveExplorer/><Mechanics/><Rewards/></main><PublicFooter/></>; }
