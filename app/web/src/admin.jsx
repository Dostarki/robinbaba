...
const ADMIN_OWNER = "0x603a26e0745ae579ad0f931307a386ddc3dd096f";
...
export default function AdminApp() {
  ...
  const [activeSaleDeploymentId, setActiveSaleDeploymentId] = useState("");
  ...
  const refreshSharedState = () => {
    API("/api/admin/state")
      .then(data => {
        ...
        setActiveSaleDeploymentId(data.activeSaleDeploymentId || "");
      })
      .catch(() => {});
  };
  ...
  const connect = async () => {
    try {
      ...
      if (address.toLowerCase() !== ADMIN_OWNER) throw Error("This wallet is not authorized for the admin panel.");
  ...
  const publishSaleTarget = async () => {
    try {
      if (!selectedDeploymentId) throw Error("Select and save a deployment before publishing it to the landing page");
      const data = await API("/api/admin/sale-target", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ deploymentId: selectedDeploymentId })
      });
      setActiveSaleDeploymentId(data.sale?.deployment?.id || selectedDeploymentId);
      setActionStatus(`Landing purchase target set to ${data.sale?.deployment?.label || deploymentLabel}.`);
      await recordActivity("Landing sale target published", "confirmed", "", selectedDeploymentId);
    } catch (err) {
      setActionStatus(`Error: ${err.message}`);
    }
  };
          ...
          <div className="admin-grid">
            ...
            <section className="admin-panel wide">
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 12 }}>
                <button className="button" style={{ width: "auto" }} onClick={publishSaleTarget} disabled={!selectedDeploymentId}>
                  {activeSaleDeploymentId === selectedDeploymentId ? "Selected for landing purchase" : "Use as landing purchase token"}
                </button>
                <span className="muted" style={{ fontSize: 11 }}>Publishing makes this saved curve and token address the only purchase target shown on the landing page.</span>
              </div>
            </section>
        ...
        {activeTab === "admins" && (
          <div>
            <section className="admin-panel">
              ...
              <p className="muted">This panel is permanently restricted to the owner wallet.</p>
