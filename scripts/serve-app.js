...
// The admin surface is intentionally a single-wallet allowlist. Do not turn this
// into a user-managed list: an authenticated owner must not be able to expand
// the panel's attack surface by enrolling additional wallets.
const ownerAddress = "0x603a26e0745aE579ad0F931307a386ddC3DD096F".toLowerCase();
...
function config() { ensureData(); const value = JSON.parse(fs.readFileSync(configPath, "utf8")); const owner = value.admins?.[ownerAddress] || { role: "owner", totp: null }; const locked = { admins: { [ownerAddress]: { role: "owner", totp: owner.totp || null } } }; if (JSON.stringify(value.admins) !== JSON.stringify(locked.admins)) save(locked); return locked; }
// Apply the one-wallet policy at boot as well, so stale administrator records
// never remain in the on-disk auth configuration between sign-in attempts.
config();
function operations() { ensureData(); const value = JSON.parse(fs.readFileSync(operationsPath, "utf8")); return { deployments: Array.isArray(value.deployments) ? value.deployments : [], activity: Array.isArray(value.activity) ? value.activity : [], activeSaleDeploymentId: typeof value.activeSaleDeploymentId === "string" ? value.activeSaleDeploymentId : null }; }
...
function requireSession(request, response) { const item = session(request); if (!item || item.address !== ownerAddress) { if (cookie(request)) sessions.delete(cookie(request)); json(response, 401, { error: "The connected wallet is not authorized for this admin panel" }); return null; } return item; }
function issueSession(response, address) { if (address !== ownerAddress) return; const token = id(); sessions.set(token, { address, expires: Date.now() + 30 * 60 * 1000 }); const secure = process.env.NODE_ENV === "production" ? "; Secure" : ""; response.setHeader("Set-Cookie", `admin_session=${token}; HttpOnly; SameSite=Strict; Path=/; Max-Age=1800${secure}`); }
...
function publicState() {
  ...
  const selected = state.deployments.find(item => item.id === state.activeSaleDeploymentId) || null;
  const network = selected?.network === "mainnet" ? { chainId: 4663, chainName: "Robinhood Chain Mainnet", rpcUrl: "https://rpc.mainnet.chain.robinhood.com", explorerUrl: "https://robinhoodchain.blockscout.com" } : selected?.network === "testnet" ? { chainId: 46630, chainName: "Robinhood Chain Testnet", rpcUrl: "https://rpc.testnet.chain.robinhood.com", explorerUrl: "https://explorer.testnet.chain.robinhood.com" } : null;
  const deployment = selected && network ? { id: selected.id, label: selected.label, tokenSymbol: selected.tokenSymbol, tokenAddress: selected.tokenAddress, curveAddress: selected.curveAddress, network: selected.network, ...network, updatedAt: selected.updatedAt } : null;
  return { sale: { configured: Boolean(deployment), source: "admin-selected-deployment", deployment }, activity: { confirmed: state.activity.some(item => item.state === "confirmed") } };
}
...
async function api(request, response, pathname) {
  ...
  if (request.method === "POST" && pathname === "/api/auth/nonce") { const { address } = await readBody(request); let account; try { account = normalize(address); } catch { return json(response, 400, { error: "Invalid wallet address" }); } if (account !== ownerAddress) return json(response, 403, { error: "This wallet is not authorized for the admin panel" }); const challengeId = id(), nonce = id(), issuedAt = new Date().toISOString(), message = `Robinity Intelligence Admin Access\nDomain: ${request.headers.host}\nAddress: ${account}\nNonce: ${nonce}\nIssued At: ${issuedAt}\nPurpose: Sign in to the Robinity Intelligence admin panel.`; challenges.set(challengeId, { account, message, expires: Date.now() + 5 * 60 * 1000 }); return json(response, 200, { challengeId, message }); }
  if (request.method === "POST" && pathname === "/api/auth/verify-wallet") { const { challengeId, signature } = await readBody(request), challenge = challenges.get(challengeId); challenges.delete(challengeId); if (!challenge || challenge.expires < Date.now()) return json(response, 401, { error: "Sign-in request expired. Start again." }); let signer; try { signer = normalize(verifyMessage(challenge.message, signature)); } catch { return json(response, 401, { error: "Invalid wallet signature" }); } if (signer !== ownerAddress || signer !== challenge.account) return json(response, 403, { error: "This wallet is not authorized for the admin panel" }); const admin = config().admins[ownerAddress]; const phase = admin.totp ? "totp" : "enroll"; return json(response, 200, { phase, ticket: createTicket(signer, phase) }); }
  if (request.method === "POST" && pathname === "/api/auth/totp-setup") { const { ticket } = await readBody(request), item = getTicket(ticket, "enroll"); if (!item || item.address !== ownerAddress) return json(response, 401, { error: "A valid owner enrollment is required" }); const admin = config().admins[ownerAddress]; if (admin.totp) return json(response, 409, { error: "Authenticator is already enrolled" }); const secret = base32(crypto.randomBytes(20)), uri = `otpauth://totp/Robinity%20Intelligence%20Admin:${item.address}?secret=${secret}&issuer=Robinity%20Intelligence%20Admin&algorithm=SHA1&digits=6&period=30`; item.secret = secret; item.phase = "setup"; const qrDataUrl = await QRCode.toDataURL(uri, { errorCorrectionLevel: "M", margin: 2, width: 260 }); return json(response, 200, { qrDataUrl }); }
  if (request.method === "POST" && pathname === "/api/auth/verify-totp") { const { ticket, code } = await readBody(request), item = getTicket(ticket, "totp") || getTicket(ticket, "setup"); if (!item || item.address !== ownerAddress) return json(response, 401, { error: "Authenticator request expired. Start again." }); const state = config(), admin = state.admins[ownerAddress], secret = item.phase === "setup" ? item.secret : admin?.totp && decrypt(admin.totp); if (!secret || !validTotp(secret, code)) return json(response, 401, { error: "Invalid authenticator code" }); if (item.phase === "setup") { admin.totp = encrypt(secret); save(state); } tickets.delete(ticket); issueSession(response, ownerAddress); return json(response, 200, { address: ownerAddress, role: "owner" }); }
  ...
  if (request.method === "POST" && pathname === "/api/admins") { const user = requireSession(request, response); if (!user) return; return json(response, 403, { error: "The administrator allowlist is locked to the owner wallet" }); }
  ...
  if (request.method === "GET" && pathname === "/api/admin/state") { const user = requireSession(request, response); if (!user) return; const state = operations(); return json(response, 200, { deployments: state.deployments, activity: state.activity, activeSaleDeploymentId: state.activeSaleDeploymentId }); }
  ...
  if (request.method === "POST" && pathname === "/api/admin/sale-target") { const user = requireSession(request, response); if (!user) return; if (!sameOrigin(request)) return json(response, 403, { error: "Cross-origin request blocked" }); const { deploymentId } = await readBody(request); const state = operations(), deployment = state.deployments.find(item => item.id === deploymentId); if (!deployment) return json(response, 404, { error: "Select a saved deployment before publishing it to the landing page" }); state.activeSaleDeploymentId = deployment.id; saveOperations(state); return json(response, 200, { sale: publicState().sale }); }
